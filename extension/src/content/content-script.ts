...
(() => {
  ...
  function domHint(element: Element): string {
    const parts: string[] = [];
  ...
  function hasEquivalentTextChild(element: Element, text: string): boolean {
  ...
  function isMeaningfulRawTextBlock(element: Element, text: string): boolean {
    const normalized = cleanText(text);
    const tagName = element.tagName.toLowerCase();
    const words = wordCount(normalized);
  ...
  function collectRawTextBlocks(limit: number): Browmate.RawTextBlock[] {
    if (!(document.body instanceof HTMLElement)) {
      return [];
    }

    const blocks: Browmate.RawTextBlock[] = [];
    const seen = new Set<string>();
  ...
  function detectRawText(bestStructuredScore = 0): DetectedCandidate | null {
