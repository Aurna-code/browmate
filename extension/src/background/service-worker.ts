...
let activeContextCache: Browmate.ActiveTabContext = emptyContext();
...
function logError(event: string, details?: unknown): void {
  if (typeof details === "undefined") {
...
function noAttachedTabContext(message = "No tab is attached. Open a normal webpage and click the Browmate toolbar action."): Browmate.ActiveTabContext {
  return {
    tabId: null,
...
function getUnsupportedPageError(url: string | undefined): string | undefined {
  if (!url) {
...
function buildContextFromTab(tab: chrome.tabs.Tab | undefined, options: { strictUrl: boolean; recordedAt?: string }): Browmate.ActiveTabContext {
  ...
  const unsupportedError = url ? getUnsupportedPageError(url) : undefined;
  if (unsupportedError) {
    return {
      tabId: null,
      recordedAt,
...
async function setActiveContext(context: Browmate.ActiveTabContext, source?: string): Promise<void> {
...
async function getStoredContext(): Promise<Browmate.ActiveTabContext> {
  ...
  activeContextCache = (result[ACTIVE_CONTEXT_KEY] as Browmate.ActiveTabContext | undefined) ?? emptyContext();
  logInfo("activeContext loaded", {
...
async function openSidePanel(windowId: number | undefined): Promise<void> {
...
async function resolveActiveTabContext(source: string): Promise<Browmate.ActiveTabContext> {
  const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
  const context = tab
    ? buildContextFromTab(tab, { strictUrl: false })
    : noAttachedTabContext();

  await setActiveContext(context, source);
  return context;
}

function isMissingContext(context: Browmate.ActiveTabContext): boolean {
  return typeof context.tabId !== "number";
}

function normalizeExtractionError(error: unknown): string {
...
chrome.action.onClicked.addListener((tab) => {
  ...
  const openPromise = openSidePanel(tab.windowId).catch((error: unknown) => {
    logError("sidePanel.open failed", error);
  });

  void Promise.all([storePromise, openPromise]).catch((error: unknown) => {
